...
class CsvImageCaptionDataset(Dataset):
    def __init__(self, csv_file, caption_csv_file, transforms, img_key='filepath'):
        
        self.df = pd.read_csv(csv_file)
        self.captions = pd.read_csv(caption_csv_file)
        self.captions['captions'] = self.captions['captions'].apply(lambda caps: eval(caps))
    ...
    def __getitem__(self, idx):
        ...
        captions = self.captions.iloc[idx]['captions']
        aff_captions = [aff_cap for aff_cap, _ in zip(captions[0], captions[1])]
        neg_captions = [neg_cap for _, neg_cap in zip(captions[0], captions[1])]

        return image, aff_captions, neg_captions
    

class CsvVideoCaptionDataset(Dataset):
    def __init__(self, csv_file, caption_csv_file, transforms, sep=',', img_key='image_id'):
    ...
    def __getitem__(self, idx):
        ...
        aff_captions = [aff_cap for aff_cap, _ in zip(captions[0], captions[1])]
        neg_captions = [neg_cap for _, neg_cap in zip(captions[0], captions[1])]
        return transformed_frames, aff_captions, neg_captions


def image_captions_collate_fn(batch):
    images, aff_texts, neg_texts = list(zip(*batch))
    images = default_collate(images)
    return images, aff_texts, neg_texts
...
@torch.no_grad()
def retreival_evaluation(model, dataLoader, len_dataLoader, device, threshold=0.9):    
...
@torch.no_grad()
def video_retreival_evaluation(model, dataLoader, len_dataLoader, device, threshold=0.9):    
    batch_images_emb_list = []
    batch_texts_emb_list = []
    texts_image_index = []
    ...
    print(f"Image retrieval at 1: {img_retrieval_1:.4g}")
    print(f"Text retrieval at 1: {txt_retrieval_1:.4g}")
    ...
    print(f"Image retrieval at 10: {img_retrieval_10:.4g}")
    print(f"Text retrieval at 10: {txt_retrieval_10:.4g}")
...
def main():
    ...
    if args.dataset == 'coco':
        dataset = CsvImageCaptionDataset(csv_file='data/images/COCO_val_negated_retrieval_llama3.1_rephrased_affneg_true.csv', caption_csv_file = "data/images/coco_retreival_mistral_captions.csv", transforms=preprocess, img_key='filepath')
        ...
        retreival_evaluation(model, dataLoader, len_dataLoader, device, args.threshold)
        
    elif args.dataset == 'msr_vtt':
        dataset = CsvVideoCaptionDataset(csv_file='data/videos/msr_vtt_retrieval_rephrased_llama.csv', caption_csv_file = "data/videos/msrvtt_retreival_mistral_captions.csv", transforms=preprocess, img_key='image_id')
        dataLoader = DataLoader(dataset, args.batch_size, collate_fn=image_captions_collate_fn, shuffle=False, num_workers=8)
        len_dataLoader = len(dataLoader)
        dataLoader = dataloader_with_indices(dataLoader)
        print("Threshold: ", args.threshold, ", Pretrained Model: ", args.pretrained_model, ", Backbone: ", args.clip_backbone, ", Dataset: ", args.dataset)
        video_retreival_evaluation(model, dataLoader, len_dataLoader, device, args.threshold)
