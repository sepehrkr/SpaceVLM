...
class CsvImageCaptionDataset(Dataset):
    def __init__(self, csv_file, transforms, img_key='filepath'):
...
@torch.no_grad()
def retreival_evaluation(model, llm, llm_tokenizer, dataLoader, len_dataLoader, device, threshold=0.9):    
    ...
    for batch_images, batch_texts, inds in tqdm(dataLoader, total=len_dataLoader):
        ...
        with torch.no_grad(), sdpa_kernel([SDPBackend.FLASH_ATTENTION, SDPBackend.EFFICIENT_ATTENTION]):
            inputs = llm_tokenizer.apply_chat_template(
                [[{'role': 'user', 'content': cap}] for captions in batch_texts for cap in captions],
...
@torch.no_grad()
def video_retreival_evaluation(model, llm, llm_tokenizer, dataLoader, len_dataLoader, device, threshold=0.9):    
    batch_images_emb_list = []
    batch_texts_emb_list = []
    texts_image_index = []
...
def main():
    ...
    if args.dataset == 'coco':
        ...
        retreival_evaluation(model, llm, llm_tokenizer, dataLoader, len_dataLoader, device, args.threshold)
        ...
        dataLoader = dataloader_with_indices(dataLoader)
        print("Threshold: ", args.threshold, ", Pretrained Model: ", args.pretrained_model, ", Backbone: ", args.clip_backbone, ", Dataset: ", args.dataset)
        video_retreival_evaluation(model, llm, llm_tokenizer, dataLoader, len_dataLoader, device, args.threshold)
